...
var questionArrayFive: [QuestionObj] = []

class SinglePlayerFive: UIViewController, ADBannerViewDelegate {
    override func viewDidLoad() {
        super.viewDidLoad()
        RandomQuestions(array: questionArrayFive)
    }
    
    func parseJson() {
        let jsonURLString = "https://whap-db.herokuapp.com/?q=TimePeriodFive"
        ...
        URLSession.shared.dataTask(with: url) { (data, response, err) in
            ...
            do {
                ...
                for question in model {
                    questionArrayFive.append(question)
                }
                
                for question in questionArrayFive {
                    print(question.qID)
                }
